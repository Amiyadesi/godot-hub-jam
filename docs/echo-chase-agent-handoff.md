...
- 用户负责正式地图、路线、机关位置、出口、最终 Camera/Phantom Camera、角色成稿和最终试玩删改。
...
├── World [PAUSABLE]
│   ├── Platform [TileMapLayer, 96 tiles]
│   ├── SpawnPoint
...
场景已按用户要求加入临时相机和完整机制展台。96 格平台、机关顺序与背景只用于逐项测试，不代表作者的正式灰盒或路线；用户可以直接在 Editor 中移动、替换或删除。
...
- 菜单地图与四类机关占位也只使用同一 Kenney atlas；菜单合成图位于 `assets/echo_chase/ui/menu_map_backdrop.png`。
...
- 用户灰盒 A、灰盒 B 和任何正式路线。
- 正式 Camera2D / Phantom Camera 参数与 CameraZone。
