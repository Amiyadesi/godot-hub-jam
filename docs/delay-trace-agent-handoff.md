...
项目显示名为 `Delay Trace`；`DelayTrace` 仍是内部开发代号、资源路径和类名。
...
入口：`res://Scenes/DelayTrace/delay_trace_start.tscn`
...
- 三种时态 Core 共用 `assets/delay_trace/character/echo_character_frames.tres`，保留角色原始像素，不再使用双色 shader 重画内部细节。
- 时态轮廓共用 `assets/delay_trace/character/echo_character_outline_frames.tres`：每帧为 `24x24px`，原 `16x16px` 内容四周各有 `4px` 透明边，避免 atlas 串帧和轮廓裁切。
...
- 平台使用 `assets/delay_trace/tilemap/echo_platform_tileset.tres`，逻辑格为 `16x16`。
...
- 菜单音乐为 Frenchyboy 的 CC0 `Mysterious2.wav` 转码版本：`assets/delay_trace/audio/music/mysterious_futuristic_loop.ogg`；来源与 SHA256 见 `docs/asset-attributions.md`。
...
$env:APPDATA = Join-Path $env:TEMP 'delay-trace-test-appdata'
& $godot --headless --editor --path . --quit
& $godot --headless --path . --scene res://Tests/DelayTrace/run_tests.tscn
& $godot --headless --path . --scene res://Tests/delay_trace_architecture_smoke.tscn
