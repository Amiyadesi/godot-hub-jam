...
当前仓库完成核心机制、独立 prefab、黑白时间轨迹菜单、设置/存档框架、自动测试，以及一个可直接试玩全部时间机关的施工入口。正式灰盒、路线、出口、最终机关摆位、正式相机参数、角色成稿和最终试玩仍由项目作者决定。
...
- 进入记录器自动录像，最长 `5s`，最多两个未来可能性槽；首帧即可提交，短录像保持末帧补足到 `1s`。
...
施工入口是 [echo_chase_start.tscn](Scenes/EchoChase/echo_chase_start.tscn)：包含一个玩家、一条 96 格平台、一个缩放为 `4×` 的测试 `Camera2D`、一个 checkpoint、`1/3/5s` 延迟台、两个记录器、已接线压力板与时间门、录制 HUD、同色时态淡入、出界复位和暂停/设置 UI。它是机制展台，不代表正式路线。
...
当前测试覆盖路径插值/回传断点、首帧回传、`1/3/5s` 延迟台、未来槽位、独立退场快照与槽位复用、时态碰撞矩阵、checkpoint 延迟恢复、首次过去体预演、录制 HUD/重绑定/低闪烁、同色入场冻结、Toggle 松开提交、Toast 鼠标穿透、暂停路由和黑白 UI authored 合同。
