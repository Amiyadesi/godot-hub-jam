...
##    `ProjectDialogueLabel.bbcode_to_plain_text(...)`
class_name ProjectDialogueLabel extends RichTextLabel

const LOCAL_EFFECTS_DIRECTORY := "res://Dialogue/Effects/Dialogue"
const RICHTEXT2_EFFECT_DIRECTORIES := [
	"res://Dialogue/Effects/RichText/effects",
	"res://Dialogue/Effects/RichText/anims"
]
...
func get_dialogue_plain_text() -> String:
	...
	return ProjectDialogueLabel.bbcode_to_plain_text(source_text)
...
## 将任意对白文本（可能包含颜色、链接、ProjectDialogueLabel 特效标签）转成纯文本。
...
func _install_enabled_text_effects() -> void:
	for effect_name in _get_requested_effect_names():
		...
		if effect == null:
			push_warning("ProjectDialogueLabel: unknown effect '%s' was skipped." % effect_name)
			continue
...
func _create_effect_instance(effect_name: String) -> RichTextEffect:
	...
	if script == null:
		push_warning("ProjectDialogueLabel: failed to load effect script '%s'." % script_path)
		return null
