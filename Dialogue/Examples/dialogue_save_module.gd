...
##
## 注意：这里会自动调用 `ProjectDialogueLabel.bbcode_to_plain_text()` 清理 BBCode 与特效标签，
...
func save_progress(
	...
	dialogue_snippet       = ProjectDialogueLabel.bbcode_to_plain_text(snippet).left(60)  # 最多保存 60 个字符的摘要
